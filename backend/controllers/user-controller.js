...
userController.prototype.post = async (req, res) => {
    ...
    _validationContract.isRequired(req.body.senha, 'Informe seu senha pentelho');
    _validationContract.isRequired(req.body.senhaConfirmacao, 'Informe seu senha confirmação pentelho');
    _validationContract.isTrue(req.body.senhaConfirmacao !== req.body.senha, 'As senha devem ser iguais pentelho');
    ...
    try{
        ...
        if(usuarioEmailExiste){
            _validationContract.isTrue(usuarioEmailExiste.nome != undefined, `Já existe o email ${req.bdy.email} cadastrado no banco de dados`)
        };
...
userController.prototype.put = async (req, res) => {
    ...
    try{
        let usuarioEmailExiste = await _repo.IsEmailExiste(req.body.email);
        if(usuarioEmailExiste){
            _validationContract.isTrue(usuarioEmailExiste.nome != undefined && usuarioEmailExiste._id != req.params.id, `Já existe o email ${req.bdy.email} cadastrado no banco de dados`)
        }
...
userController.prototype.get = async(req, res) => {
    _validationContract.isRequired(req.paramsir, 'Informe seu od pentelho');
