...
class userRepository {
    ...
    async update(id, data, usuarioLogado) {
        if(usuarioLogado._id === id) {
            if(data.oldPPassword !== data.senha && data.oldPPassword && data.senha !== undefined && data.senhaConfirmacao !== undefined && data.senha === data.senhaConfirmacao){
                let user = await this._base._model.findOne({_id:id});
                if(await bcrypt.compareSync(data.oldPPassword, user.senha)){
